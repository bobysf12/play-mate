...
function formatDate(d: Date) {
	...
	return (
		<div>
			<div style={{ fontSize: 12 }}>{dst[0]}</div>
			<div style={{ fontSize: 16, fontWeight: 600 }}>{dst[2]}</div>
			<div style={{ fontSize: 12, fontWeight: 300 }}>{dst[1]}</div>
		</div>
	);
}
	...
	return (
		...
		>
			...
			>
				...
				>
					{props.act <= 0 ? "No" : props.act} activity
				</span>
			</div>
		</div>
	);
		...
		>
			...
			>
				<g transform="translate(4.033325, 0.166667)">
					<path d="M0.488888889,7.33333333 L7.33333333,0.488888889" />
					<path d="M0.488888889,7.33333333 L7.33333333,14.1777778" />
				</g>
			</svg>
		</div>
	);
		...
		>
			...
			>
				<g transform="translate(4.033325, 0.166667)">
					...
					<path d="M7.33333333,7.33333333 L0.488888889,14.1777778" />
				</g>
			</svg>
		</div>
	);
...
export default class DateFilter extends React.Component {
	...
	render() {
		const dates = lodash.map(lodash.range(3), x => {
			...
			return (
				<DateItem
					...
					active={this.state.selectedIdx === d}
					disabled={false}
				/>
			);
		});
		...
		return (
			<div style={{ flexDirection: "row", display: "flex", padding: 4, cursor: "pointer" }}>
