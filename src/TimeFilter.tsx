...
export default class TimeRange extends React.Component {
	...
	moveLeft = (e: MouseEvent) => {
		...
		this.setState({
			left:
				Math.max(0, Math.min(this.state.right / 100, (clientX - this.state.leftOffset) / this.state.width)) *
				100,
		});
	};
	...
	moveRight = (e: MouseEvent) => {
		...
		this.setState({
			right:
				Math.max(this.state.left / 100, Math.min(1, (clientX - this.state.leftOffset) / this.state.width)) *
				100,
		});
	};
	...
	moveMid = (e: MouseEvent) => {
		...
		const trueOffset = Math.min(
			100 - this.state.right,
